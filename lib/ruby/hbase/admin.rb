...
include Java
java_import java.utils.Arrays
java_import org.apache.hadoop.hbase.utils.Pair
java_import org.apache.hadoop.hbase.utils.RegionSplitter
  ...
  class Admin
      ...
      args.each do |arg|
        ...
        if arg.has_key?(SPLITS)
          ...
          arg.delete(SPLITS).each do |split|
            splits[idx] = org.apache.hadoop.hbase.utils.Bytes.toBytesBinary(split)
            idx = idx + 1
          end
    ...
    def truncate_preserve(table_name, conf = @conf)
      ...
      begin
        ...
        splits = org.apache.hadoop.hbase.utils.Bytes.toBinaryByteArrays(splits)
      ensure
        locator.close()
      end
    ...
    def status(format, type)
      ...
      if format == "detailed"
        ...
        master_coprocs = java.utils.Arrays.toString(@admin.getMasterCoprocessors())
        ...
        if (!@admin.getConfiguration().getBoolean(org.apache.hadoop.hbase.HConstants::REPLICATION_ENABLE_KEY,
          ...
          for server in status.getServers()
            ...
            rSinkString << ", TimeStampsOfLastAppliedOp=" +
                (java.utils.Date.new(rLoadSink.getTimeStampsOfLastAppliedOp())).toString()
            ...
            while index < rLoadSourceList.size()
              ...
              rSourceString << ", TimeStampsOfLastShippedOp=" +
                  (java.utils.Date.new(rLoadSource.getTimeStampOfLastShippedOp())).toString()
    ...
    def online(region_name, on_off)
      ...
      hri = org.apache.hadoop.hbase.utils.Writables.getWritable(hri_bytes, org.apache.hadoop.hbase.HRegionInfo.new)
      hri.setOffline(on_off)

      # Write it back
      put = org.apache.hadoop.hbase.client.Put.new(region_bytes)
      put.add(org.apache.hadoop.hbase.HConstants::CATALOG_FAMILY, org.apache.hadoop.hbase.HConstants::REGIONINFO_QUALIFIER, org.apache.hadoop.hbase.utils.Writables.getBytes(hri))
      meta.put(put)
    end
    ...
    def list_namespace(regex = ".*")
      pattern = java.utils.regex.Pattern.compile(regex)
    ...
    def clear_deadservers(dead_servers)
      ...
      if dead_servers.empty?
        ...
        servers = java.utils.ArrayList.new
