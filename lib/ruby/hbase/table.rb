...
EOF
    ...
    def _incr_internal(row, column, value = nil, args={})
      ...
      org.apache.hadoop.hbase.utils.Bytes::toLong(cell.getValue)
    end
    ...
    def _append_internal(row, column, value, args={})
      ...
      # Fetch cell value
      cell = result.listCells[0]
      org.apache.hadoop.hbase.utils.Bytes::toStringBinary(cell.getValue)
    end
    ...
    def _count_internal(interval = 1000, caching_rows = 10)
      ...
      while iter.hasNext
        ...
        yield(count,
              org.apache.hadoop.hbase.utils.Bytes::toStringBinary(row.getRow))
      end
    ...
    def _get_internal(row, *args)
      ...
      result.list.each do |kv|
        ...
        qualifier = org.apache.hadoop.hbase.utils.Bytes::toStringBinary(kv.getQualifier)
    ...
    def _scan_internal(args = {}, scan = nil)
      ...
      while iter.hasNext
        row = iter.next
        key = org.apache.hadoop.hbase.utils.Bytes::toStringBinary(row.getRow)
      ...
      if is_meta_table?
        ...
        if column == 'info:serverstartcode'
          if kv.getValue.length > 0
            str_val = org.apache.hadoop.hbase.utils.Bytes.toLong(kv.getValue)
          else
            str_val = org.apache.hadoop.hbase.utils.Bytes.toStringBinary(kv.getValue)
          end
    ...
    def convert(column, kv)
      #use org.apache.hadoop.hbase.utils.Bytes as the default class
      klazz_name = 'org.apache.hadoop.hbase.utils.Bytes'
      #use org.apache.hadoop.hbase.utils.Bytes::toStringBinary as the default convertor
